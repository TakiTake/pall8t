...
- Superseded in part by: [ADR-0009](0009-explicit-mounts.md) — the revisit trigger below has fired. `[[repos]]` and its `git clone --local` duplication are gone, replaced by `[[mounts]]`: any directory, mounted literally, read-only on request.
...
- **Reference repos keep ADR-0004's protection insight.** Repos listed in `pall8t.toml` are duplicated via `git clone --local` (hardlinked objects) and the copy is mounted — protection by duplication, still compensating for apple/container's missing RO mounts ([apple/container#990](https://github.com/apple/container/issues/990)). *(Superseded by [ADR-0009](0009-explicit-mounts.md): #990 had already closed when this was written, so the premise was wrong even then. The clone is gone; `[[mounts]]` mounts any directory directly, read-only on request.)*
...
- ~~apple/container#990 (RO mounts) resolved → mount reference repos ro directly (roadmap item 2).~~ **Fired** — #990 had in fact closed 2026-01-07, before this ADR was written. Done in [ADR-0009](0009-explicit-mounts.md).
