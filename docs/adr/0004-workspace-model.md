...
- Status: Amended by ADR-0006 (project-workspace model dropped; hardlink-clone protection retained), then by [ADR-0009](0009-explicit-mounts.md)
- Date: 2026-07-04
- **The "no read-only mounts" constraint below is obsolete.** apple/container#990 closed 2026-01-07, before 1.0.0 shipped; read-only enforcement is verified on 1.2.2, and [ADR-0009](0009-explicit-mounts.md) replaces the cloning entirely with `[[mounts]]` — any directory, mounted literally, `readonly = true` where protection is wanted. The identity-path mount insight this ADR introduced is unaffected and is now the default target. The deferred `--reference` alternates design is moot: it belonged to the workspace seeding that ADR-0006 removed.
