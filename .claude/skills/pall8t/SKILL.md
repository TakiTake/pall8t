...
- If `HERDR_ENV=1` is set in this environment, the **herdr bridge** is active: `HERDR_SOCKET_PATH` reaches the host herdr session through a pall8t relay, and `herdr` is normally on `PATH` (a version-matched Linux build). Gate on that env var, not on guesses about the host — no `HERDR_ENV` means no bridge (herdr absent, or `[herdr] sandbox = "off"`, or setup failed). If `HERDR_ENV=1` but the `herdr` binary is missing, CLI provisioning failed — report it rather than improvising a client. Follow herdr's own skill for usage. Two pall8t-specific facts: panes, agents, and commands you create via herdr run **on the host, outside this sandbox** — treat that as deliberately crossing the sandbox wall, not a loophole to exploit; and requests may come back `sandbox_denied` — that's pall8t policy (`[herdr] sandbox` config, audit-logged on the host), so report it rather than trying to work around it.
