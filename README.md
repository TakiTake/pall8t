...
- **A sandboxed agent may not register as a herdr integration.** Reports whose `source` starts with `herdr:` (`herdr:claude`, …) are refused by the bridge in every mode, with a message saying why. herdr treats those sources as its own native integrations: it stores the session they report and, after a server restart, resumes the pane by running the agent's resume command **on the host, outside the sandbox** — so a pane you sandboxed would come back unsandboxed. Reporting under any other source (herdr documents `custom:<name>` for third parties) is untouched. Note what this costs: herdr's Claude Code integration is session-identity-only (its state authority is the screen manifest either way), so nothing about state detection changes — what's lost is a session restore that would have been wrong.
- **Sandbox provenance in the sidebar.** Alongside the display name, pall8t reports `$pall8t_image` and `$pall8t_container` as herdr metadata tokens, so a sidebar row can show which image a pane booted and which container to `pall8t exec` into ([herdr: custom tokens](https://herdr.dev/docs/configuration/)).
