...
- **Naming the tab and the agent** (opt-in, issue #71; numbering scheme issue #76). A herdr agent's *name* is what makes cross-sandbox agent-to-agent messaging usable — `herdr agent prompt api-2 "run the tests" --wait`. Without one the only working target is the pane id (`w13:p3`), which changes every run and reads like nothing: tab labels don't resolve as agent targets, and neither do tab ids (verified on herdr 0.8.2). With `auto_rename = true` under `[herdr]`, `pall8t run` names **both** the tab and the agent with the same string, so what you read off the tab is what you type. The name is the workspace directory's basename, slugged, plus the tab's number — `~/src/foo` as the first tab in its workspace becomes `foo-1`; `[herdr] agent_name = "api"` replaces the basename (`api-1`). The suffix is the tab's **position** in its own workspace — the same 1-based number herdr's own default label already shows on an unrenamed tab — recomputed fresh on every run, so it reads the same number a human would see if pall8t had done nothing here. A name already claimed gets a further counter (`foo-1-2`), so a target always resolves to exactly one agent and no two tabs read the same name. Both kinds of claim count: a name a live agent answers to, and a label another tab already wears — a tab keeps the label it was given at the position it held *then*, so closing an earlier tab can leave a label sitting on exactly the name a later tab's new position would derive. **Off unless you set it**: `auto_rename` undefined means pall8t renames nothing, and `agent_name` alone doesn't switch it on — that combination warns on stderr rather than silently doing nothing. A tab *you* renamed is never clobbered; one still on herdr's own label — or carrying a label pall8t wrote there on an earlier run, at its current position or an earlier one (an earlier tab closing shifts positions, which the recognition now tolerates) — is taken over, so a second run in the same tab can't leave the label pointing at some other run's agent. The auto label is inferred from it being the tab's position in its workspace, since herdr's API reports the label but not whether it is the automatic one, so the single case pall8t can't tell apart is a tab you renamed to exactly its own position number, or to exactly a name pall8t would have picked here. When the label really is someone else's, the run says which name reaches the agent and which one the tab keeps. The two halves land at different times: the tab is renamed before the sandbox even starts (it needs no detected agent), while the agent can only be renamed once herdr recognizes it — which happens after pall8t has exec'd into `container run` — so a small detached `pall8t` child waits for that and renames it then, logging to `~/.pall8t/logs/herdr-naming.log`. Best-effort and independent: a failing agent rename never stops the tab from being named, and naming happens in every `[herdr] sandbox` mode, `off` included.
