...
- **Naming the tab and the agent** (opt-in, issue #71; numbering scheme [ADR-0011](docs/adr/0011-tab-numbering-state.md)). A herdr agent's *name* is what makes cross-sandbox agent-to-agent messaging usable — `herdr agent prompt api-2 "run the tests" --wait`. Without one the only working target is the pane id (`w13:p3`), which changes every run and reads like nothing: tab labels don't resolve as agent targets, and neither do tab ids (verified on herdr 0.8.2). With `auto_rename = true` under `[herdr]`, `pall8t run` names **both** the tab and the agent with the same string, so what you read off the tab is what you type. The name is the workspace directory's basename, slugged, plus a number — `~/src/foo` as the first tab of a fresh herdr session becomes `foo-1`; `[herdr] agent_name = "api"` replaces the basename (`api-1`). The number is **pall8t's own counter**, kept in `~/.pall8t/state/herdr-naming.json` and counted per base name: handed out once, never reused while one herdr server run lasts, and started over when the server restarts ([ADR-0011](docs/adr/0011-tab-numbering-state.md)). So a tab keeps its name for its whole life — closing another tab never renumbers it, which is what makes the name usable as an address other agents type. herdr restores tab labels across a server stop, so the reset is not blind: a tab herdr brought back keeps the number its own label carries instead of being renamed, and a restarted counter starts past the labels still on screen, landing on 1 only when nothing is left wearing one. The counter is tied to a server run by the API socket's identity, because herdr exposes no session id, pid or start time of its own; if that cannot be read, the count simply carries on rather than risking a reset onto names that are still in use. Numbering needs no herdr call at all, so a `tab.list` that fails costs the tab its rename but never costs the agent its number. Lost the file? The next run seeds itself from the labels on screen, so deleting `~/.pall8t/state/herdr-naming.json` is the way to start the numbering over. A name already claimed gets a further counter (`foo-1-2`), so a target always resolves to exactly one agent and no two tabs read the same name. Both kinds of claim count: a name a live agent answers to, and a label another tab already wears. The second matters because herdr enforces no uniqueness on labels and restores them across a server stop, so two tabs really can end up wearing one name — and a tab whose agent has exited is invisible to `agent.list`, which is exactly how it happened live. **Off unless you set it**: `auto_rename` undefined means pall8t renames nothing, and `agent_name` alone doesn't switch it on — that combination warns on stderr rather than silently doing nothing. A tab *you* renamed is never clobbered; one still on herdr's own label — or carrying a label pall8t wrote there on an earlier run, at whatever number that run was up to — is taken over, so a second run in the same tab can't leave the label pointing at some other run's agent. The number in that old label is deliberately not compared against the current one: after a server restart every surviving tab's label carries a number from the previous run's sequence, and requiring them to match would hand every one of those tabs back to "someone else's label". The auto label is inferred from it being the tab's position in its workspace, since herdr's API reports the label but not whether it is the automatic one, so the single case pall8t can't tell apart is a tab you renamed to exactly its own position number, or to exactly a name pall8t would have picked for this project at some number. When the label really is someone else's, the run says which name reaches the agent and which one the tab keeps. The two halves land at different times: the tab is renamed before the sandbox even starts (it needs no detected agent), while the agent can only be renamed once herdr recognizes it — which happens after pall8t has exec'd into `container run` — so a small detached `pall8t` child waits for that and renames it then, logging to `~/.pall8t/logs/herdr-naming.log`. Best-effort and independent: a failing agent rename never stops the tab from being named, and naming happens in every `[herdr] sandbox` mode, `off` included.
